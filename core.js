...
function utmbCategoryToYearRange(cat) {
  ...
  const buffer = 2; // marge pour absorber l'incertitude sur l'année exacte de la course
  let m = c.match(/^(\d{2})\s*-\s*(\d{2})$/);
  if(m) {
    const minAge = +m[1], maxAge = +m[2];
    return { min: refYear - maxAge - buffer, max: refYear - minAge + buffer };
  }
  m = c.match(/^(\d{2})\s*\+$/);
  if(m) {
    const minAge = +m[1];
    return { min: 1900, max: refYear - minAge + buffer };
  }
  return null;
}
...
function ffaCategoryToYearRange(cat) {
  ...
  const m = c.match(/^(U\d{1,2}|ES|SE|JU|CA|MI|BE|PO|EA|M\d{1,2})/);
  if(!m) return null;
  const range = FFA_CATEGORY_AGE_TABLE[m[1]];
...
function parsePlainText(text) {
  ...
  for(const line of lines) {
    ...
    const cat = /^(U\d{1,2}|ES|SE|JU|CA|MI|BE|PO|EA|M\d{1,2})[HF]?$/i.test(after) ? after.toUpperCase() : "";
